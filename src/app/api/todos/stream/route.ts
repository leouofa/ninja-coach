import { listOpenTodosWithGoal } from "@/lib/db/queries";
  ...
  const stream = new ReadableStream<Uint8Array>({
    start(controller) {
      ...
      function sendSnapshot() {
        if (closed) return;
        try {
          const snapshot = JSON.stringify(listOpenTodosWithGoal());
          if (snapshot === previous) return;
          previous = snapshot;
          controller.enqueue(
            encoder.encode(`event: todos\ndata: ${snapshot}\n\n`),
          );
        } catch (error) {
          console.error("[todos:stream] polling failed:", error);
          cleanup();
        }
      }
